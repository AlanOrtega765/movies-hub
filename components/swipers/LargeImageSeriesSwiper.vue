  ...
  >
    <SwiperSlide v-for="slide in swiperSlides" :key="slide.id">
      ...
      >
        ...
        <p
          class="text-sm tablet:text-md mt-1 tablet:w-4/5 laptop:hidden font-light"
        >
          {{ slide.overview.slice(0, 150).trim() + '...' }}
        </p>
        <p
          class="text-lg w-4/5 desktop:w-3/5 hidden laptop:flex font-light leading-6"
        >
          {{
        ...
        <NuxtLink
          ...
          :to="`/series/${slide.id}-${useSlug(slide.title)}`"
          >Ver Más</NuxtLink
        >
      </div>
    </SwiperSlide>
